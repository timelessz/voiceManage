...
function zhuang_editauthority(href, classId) {
    var title = '编辑用户权限信息';
    openDialog(classId, href, title);
}
/**
 ...
 * 删除信息
 */
function zhuang_allowseal(id, href, classId) {
    var title = '批准印章申请';
    $.messager.confirm(title, '确定要批准印章申请吗?', function (status) {
        if (!status) {
            return false;
        }
        $.ajax({
 ...
 * @returns null
 */
function bulkallowseal(ids, title, href, classId) {
    $.messager.confirm(title, '确人要允许公章申领吗?', function (r) {
        if (!r) {
            return false;
        }
        $.ajax({
...
function ajaxTraverse(href, type)
{
    if (type == "2")
    {
        if (!window.confirm('全部更新要花费较多时间，确认全部更新？'))
            return;
    }
    $.ajax({
        url: href,
